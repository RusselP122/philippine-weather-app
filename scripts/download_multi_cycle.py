...
def get_latest_valid_run():
    ...
    for d in dates:
        for h in hours_desc:
            url = f"{base}/FNV3_{d.strftime('%Y_%m_%d')}T{h}_00_cyclogenesis.csv"
            try:
                ...
                continue
    raise RuntimeError("No available FNV3 runs found in the last 3 days.")
...
def main():
    ...
    # Calculate 4 cycles: latest, latest - 6h, latest - 12h, latest - 18h
    cycles = [latest_dt - timedelta(hours=i*6) for i in range(4)]
    ...
    # Base and Large URLs
    BASE_TRACKS_URL = "https://deepmind.google.com/science/weatherlab/download/cyclones/FNV3/ensemble/cyclogenesis/csv"
    BASE_PAIRED_URL = "https://deepmind.google.com/science/weatherlab/download/cyclones/FNV3/ensemble_mean/paired/csv"
    ...
    for dt in cycles:
        ...
        # 1. Base Tracks
        base_tracks_fn = f"fnv3_base_{date_str}T{hour_str}_00.dat"
        base_tracks_url = f"{BASE_TRACKS_URL}/FNV3_{date_str}T{hour_str}_00_cyclogenesis.csv"
        
        # 2. Base Paired
        base_paired_fn = f"fnv3_paired_{date_str}T{hour_str}_00.dat"
        base_paired_url = f"{BASE_PAIRED_URL}/FNV3_{date_str}T{hour_str}_00_paired.csv"
