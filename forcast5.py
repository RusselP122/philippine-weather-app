...
OUTPUT_IMAGE = os.path.join(IMAGES_DIR, "tropical_outlook_week1_latest.png")
OUTPUT_JSON = os.path.join(DATA_DIR, "tropical_outlook_week1.json")
...
ALL_CITIES = [
    ...
    ("LEGAZPI", 123.73, 13.14, (0.65, 0.25)),
    # Visayas
...
def compute_dynamic_extent(areas, active_tcs=None):
    """
    Computes a threat-focused 16:9 widescreen bounding box.
    ...
    # Priority 1: Focus on potential cyclogenesis development areas
    for a in areas:
    ...
    # If no formation areas exist, check active TCs near PAR/Philippines (< 26N, < 140E)
    if not all_lons and active_tcs:
        for tc in active_tcs:
            for poly in tc.get("polygons", []):
                for pt in poly:
                    if pt[0] <= 26.0 and pt[1] <= 140.0:
                        all_lats.append(pt[0])
                        all_lons.append(pt[1])

    # If calm (no areas), fall back to standard National PAR 16:9 view
    if not all_lons:
    ...
    # Anchor relevant Philippine coastal landmass for immediate geographic orientation
    ref_points = []
    if center_threat_lon < 123.0:
        # Threat in West Philippine Sea: anchor Western Luzon, Manila, Bicol
        ref_points.extend([(120.6, 18.2), (120.6, 16.4), (121.0, 14.6), (120.3, 16.0), (121.0, 13.8), (123.7, 13.1)])
    if center_threat_lon > 122.0:
        # Threat in Philippine Sea / East: anchor Eastern seaboard
    ...
    # Lock strictly to 16:9 aspect ratio
    ...
    # If threat is in the west, shift center slightly rightward so the floating card on the left doesn't obscure the threat
...
# ── Main Execution ──────────────────────────────────────────────────────────
def main():
    print("=== Google WeatherNext 3 (WNv3) 7-Day Tropical Weather Outlook ===")
    ...
    two_day_day = (latest_ph + timedelta(days=2)).strftime("%a")
    seven_day_day = (latest_ph + timedelta(days=7)).strftime("%a")

    # Convert lead time if needed
    ...
    print(f"Total potential genesis points in WP: {len(genesis_data)}")

    # ── Active Tropical Cyclones Geometry ─────────────────────────────────────
    if active_wp_ids:
        for tid in active_wp_ids:
            tc_pts = wp_data[(wp_data["track_id"] == tid) & (wp_data["lead_time_hours"] <= 168)]
            mask_tc = (
                (tc_pts["lon"] >= 105.0) & (tc_pts["lon"] <= 165.0) &
                (tc_pts["lat"] >= 0.0) & (tc_pts["lat"] <= 38.0)
            )
            tc_pts = tc_pts[mask_tc]
            if not tc_pts.empty:
                has_active_tc = True
                area_color = "#d946ef"
                ...
                for lt in lead_times:
                    ...
                    coords = np.column_stack((pts["lon"].values, pts["lat"].values))
                    if len(coords) == 1:
                        hour_polys.append(MultiPoint(coords).buffer(0.8))
                    else:
                        hour_polys.append(MultiPoint(coords).convex_hull.buffer(0.8, join_style="round"))

                buffered_geom = unary_union(hour_polys) if hour_polys else Circle((center_lon, center_lat), 2.0)
                tc_polys_json = []

                if isinstance(buffered_geom, Polygon):
                    ext = list(buffered_geom.exterior.coords)
                    tc_polys_json.append([[lat, lon] for lon, lat in ext])
                elif hasattr(buffered_geom, "geoms"):
                ...
                json_active_tcs.append({
                    ...
                    "color": area_color,
                    "polygons": tc_polys_json,
                    "label": f"Active Cyclone ({tid})"
                })
    ...
    if len(genesis_data) >= 2:
        ...
        for i, lbl in enumerate(unique_labels, start=1):
            ...
            prob_2day_rounded = round_prob(prob_2day)
            prob_7day_rounded = round_prob(prob_7day)

            cat_2day = get_category(prob_2day_rounded)
            cat_7day = get_category(prob_7day_rounded)
            area_color = get_area_color(prob_7day_rounded)
            ...
            area_text = f"Area {i}: {prob_2day_rounded}% (48h) · {prob_7day_rounded}% (7-Day)"
            summary_line = f"Area {i}: {cat_7day.title()} potential ({prob_7day_rounded}%) of tropical cyclone formation within 7 days."

            json_areas.append({
                "id": i,
                "probability_2day": prob_2day_rounded,
                "probability_7day": prob_7day_rounded,
            ...
            display_areas.append({
                "id": i,
                "prob_2day": prob_2day_rounded,
                "prob_7day": prob_7day_rounded,
                "color": area_color,
                "cat": cat_7day,
                "center": [center_lat, center_lon],
                "polygons": area_polys_json
    ...
    # Coastlines & International Borders
    ...
    # ── Active Tropical Cyclones Plotting ─────────────────────────────────────
    for tc in json_active_tcs:
        for poly in tc["polygons"]:
            coords = [[lon, lat] for lat, lon in poly]
            ax.add_patch(MplPolygon(coords, facecolor=tc["color"], edgecolor=tc["color"], linewidth=2.8, linestyle="--", alpha=0.35, transform=ccrs.PlateCarree(), zorder=10))
        center_lat, center_lon = tc["center"]
        if extent[0] <= center_lon <= extent[1] and extent[2] <= center_lat <= extent[3]:
            badge_offset_y = -2.5 if center_lat > (extent[3] - 4.0) else 2.5
            ax.text(center_lon, center_lat + badge_offset_y, f"ACTIVE SYSTEM ({tc['track_id']})", fontsize=9.5, fontweight="heavy",
                    color="#ffffff", ha="center", va="center", transform=ccrs.PlateCarree(),
                    bbox=dict(boxstyle="round,pad=0.4", facecolor="#1e1b4b", edgecolor=tc["color"], lw=1.5, alpha=0.9), zorder=20)
    ...
    for a in display_areas:
        ...
        # Intelligent area badge placement avoiding UI card on top-left
    ...
    for name, clon, clat, (ox, oy) in ALL_CITIES:
        ...
        if (extent[0] + 0.5 <= clon <= extent[1] - 0.5) and (extent[2] + 0.5 <= clat <= extent[3] - 0.5):
            ...
            if norm_x < 0.35 and norm_y > 0.48:
                continue  # Skip city pin if obscured behind floating development card
            if norm_x > 0.65 and norm_y < 0.20:
                continue  # Skip city pin if obscured behind bottom-right legend card
            if norm_x < 0.13 and norm_y < 0.16:
                continue  # Skip city pin if obscured behind bottom-left logo badge
    ...
    # Red inner accent badge for "WEATHER"
    ...
    fig.text(0.978, 0.956, "7-DAY TROPICAL WEATHER OUTLOOK", fontsize=13.5, fontweight="heavy", color="#38bdf8", ha="right", va="center", zorder=44)
    ...
    if len(display_areas) == 1:
        ...
        for idx, a in enumerate(table_areas[:num_rows]):
            ...
            # Area Color Dot
            dot = patches.Circle((c_area_x + 0.005, curr_row_y + 0.006) if 'curr_row_y' in locals() else (c_area_x + 0.005, (start_y - idx * row_height) + 0.006), 0.0045,
                                transform=fig.transFigure, facecolor=a["color"], edgecolor="#0f172a", lw=0.6, zorder=53)
            curr_row_y = start_y - (idx * row_height)
            dot.set_center((c_area_x + 0.005, curr_row_y + 0.006))
    ...
    fig.text(leg_x + leg_w / 2.0, leg_y + 0.092, "7-DAY FORMATION POTENTIAL",
             fontsize=9.5, fontweight="heavy", color="#f8fafc", ha="center", va="center", zorder=52)

    # Category indicators (Balanced & Centered with ample border margins)
    c1 = leg_x + leg_w * 0.19
    c2 = leg_x + leg_w * 0.50
    c3 = leg_x + leg_w * 0.81
