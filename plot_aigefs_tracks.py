...
if __name__ == "__main__":
    ...
    if os.path.exists(csv_file):
        ...
        csv_bytes = bytearray([xored[i] ^ key[i % len(key)] for i in range(len(xored))])
        csv_text = csv_bytes.decode('utf-8')
        data = pd.read_csv(io.StringIO(csv_text))
    else:
        print(f"File not found: {csv_file} or {enc_file}")
        exit()
    ...
    title_5day = f"NOAA AI-GEFS {days_5day_str}-Day Forecast Tropical Cyclone Tracks\nWestern Pacific ({forecast_start} to {end_5day})"
    ...
    title_15day = f"NOAA AI-GEFS {days_15day_str}-Day Forecast Tropical Cyclone Tracks\nWestern Pacific ({forecast_start} to {end_15day})"
