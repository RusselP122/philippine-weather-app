...
// Forcast.py  → public/data/fnv3p2_latest.dat       (base ensemble members)
...
// ── Geographic Center on Unit Sphere ──────────────────────────────────────
function meanGeoCenter(points) {
...
// ── Nearest-Neighbor Track Stitching (TempestExtremes StitchNodes Logic) ───
function stitchMemberTracks(memberRows) {
    const byHour = {};
    ...
    for (const h of hours) {
        ...
        for (let tIdx = 0; tIdx < activeTracks.length; tIdx++) {
            ...
            let guessLat = lastPt.lat;
            let guessLon = lastPt.lon;

            if (track.length >= 2) {
                const prevPt = track[track.length - 2];
                const prevDt = lastPt.h - prevPt.h;
                if (prevDt > 0) {
                    const vLat = (lastPt.lat - prevPt.lat) / prevDt;
                    let dLon = lastPt.lon - prevPt.lon;
                    if (dLon > 180) dLon -= 360;
                    if (dLon < -180) dLon += 360;
                    const vLon = dLon / prevDt;

                    guessLat = lastPt.lat + vLat * dt;
                    guessLon = lastPt.lon + vLon * dt;
                    if (guessLon > 180) guessLon -= 360;
                    if (guessLon < -180) guessLon += 360;
                    guessLat = Math.max(-90, Math.min(90, guessLat));
                }
            }

            const maxDistKm = Math.min(800, Math.max(400, 100 * dt));

            for (let cIdx = 0; cIdx < candidates.length; cIdx++) {
                const cand = candidates[cIdx];
                const distKm = haversineKm({ lat: guessLat, lon: guessLon }, cand);
                if (distKm <= maxDistKm) {
                    const dp = isNaN(lastPt.p) || isNaN(cand.p) ? 0 : Math.abs(lastPt.p - cand.p);
                    const score = distKm / 100 + dp;
                    matches.push({ tIdx, cIdx, score, dist: distKm });
                }
            }
        }
        ...
        const newActiveTracks = [];

        for (const m of matches) {
            if (!matchedTracks.has(m.tIdx) && !matchedCandidates.has(m.cIdx)) {
                matchedTracks.add(m.tIdx);
                matchedCandidates.add(m.cIdx);
                const track = activeTracks[m.tIdx];
                track.push(candidates[m.cIdx]);
                newActiveTracks.push(track);
            }
        }
        ...
        for (let cIdx = 0; cIdx < candidates.length; cIdx++) {
            if (!matchedCandidates.has(cIdx)) {
                newActiveTracks.push([candidates[cIdx]]);
            }
        }
    ...
    return closedTracks.filter(t => t.length >= 2);
}

// ── Pairwise Track Distance (Mean Great-Circle Separation) ──────────────────
function getTrackDistance(trackA, trackB) {
    const pointsA = {};
    for (const p of trackA) pointsA[p.h] = p;
    let sumDist = 0, count = 0;
    for (const pB of trackB) {
        const pA = pointsA[pB.h];
        if (pA) {
            sumDist += haversineKm(pA, pB);
            count++;
        }
    }
    return count === 0 ? 5000 : sumDist / count;
}

// ── HDBSCAN Clustering Algorithm ──────────────────────────────────────────
function runHDBSCAN(distanceMatrix, minClusterSize) {
    const n = distanceMatrix.length;
    if (n === 0) return [];

    const k = Math.min(n - 1, Math.max(1, minClusterSize - 1));
    const coreDist = new Array(n);
    for (let i = 0; i < n; i++) {
        const sortedDists = [...distanceMatrix[i]].sort((a, b) => a - b);
        coreDist[i] = sortedDists[k];
    }
    ...
    for (let step = 0; step < n; step++) {
        ...
        inMST[u] = true;
        if (parent[u] !== -1) {
            edges.push({ u: parent[u], v: u, weight: best });
        }
        for (let v = 0; v < n; v++) {
            if (!inMST[v]) {
                const weight = Math.max(coreDist[u], coreDist[v], distanceMatrix[u][v]);
                if (weight < minDist[v]) {
                    minDist[v] = weight;
                    parent[v] = u;
                }
    ...
    let nextNodeId = n;
    const ufParent = Array.from({ length: n * 2 }, (_, i) => i);
    const nodes = Array.from({ length: n * 2 }, (_, i) => ({
        id: i, left: null, right: null, weight: 0, birth: 0, death: Infinity,
        points: i < n ? [i] : []
    }));
    ...
    for (const edge of edges) {
        ...
        if (rootU !== rootV) {
            ...
            ufParent[rootU] = newId;
            ufParent[rootV] = newId;
            nodes[newId].left = nodes[rootU];
            nodes[newId].right = nodes[rootV];
            nodes[newId].weight = edge.weight;
            nodes[newId].points = [...nodes[rootU].points, ...nodes[rootV].points];
            nodes[rootU].death = edge.weight;
            nodes[rootV].death = edge.weight;
        }
    }

    const rootNodeId = nextNodeId - 1;
    if (rootNodeId < n) return new Array(n).fill(0);

    let nextClusterId = 1;
    const condensedNodes = {};

    function condense(nodeId, parentClusterId) {
        const node = nodes[nodeId];
        if (!node.left && !node.right) return;
        const left = node.left, right = node.right;
        const leftCount = left.points.length, rightCount = right.points.length;

        if (leftCount >= minClusterSize && rightCount >= minClusterSize) {
            const leftClusterId = nextClusterId++;
            const rightClusterId = nextClusterId++;
            condensedNodes[leftClusterId] = {
                id: leftClusterId, parent: parentClusterId, birth: node.weight, death: left.death,
                points: left.points, stability: 0, selected: false
            };
            condensedNodes[rightClusterId] = {
                id: rightClusterId, parent: parentClusterId, birth: node.weight, death: right.death,
                points: right.points, stability: 0, selected: false
            };
            condense(left.id, leftClusterId);
            condense(right.id, rightClusterId);
        } else if (leftCount >= minClusterSize) {
            condense(left.id, parentClusterId);
        } else if (rightCount >= minClusterSize) {
            condense(right.id, parentClusterId);
        }
    }

    const rootClusterId = 0;
    condensedNodes[rootClusterId] = {
        id: rootClusterId, parent: null, birth: 0, death: nodes[rootNodeId].weight,
        points: nodes[rootNodeId].points, stability: 0, selected: false
    };
    condense(rootNodeId, rootClusterId);

    for (const cidStr of Object.keys(condensedNodes)) {
        const cid = parseInt(cidStr);
        const cnode = condensedNodes[cid];
        const lambdaBirth = 1 / (cnode.birth || 0.0001);
        let sumStability = 0;
        for (const pt of cnode.points) {
            let deathWeight = cnode.death;
            let curr = nodes[pt];
            while (curr && curr.death < cnode.death) {
                deathWeight = curr.death;
                curr = nodes[ufParent[curr.id]];
            }
            const lambdaDeath = 1 / (deathWeight || 0.0001);
            sumStability += Math.max(0, lambdaDeath - lambdaBirth);
        }
        cnode.stability = sumStability;
    }

    const clusterIds = Object.keys(condensedNodes).map(Number).sort((a, b) => b - a);
    const subtreeStability = {};
    for (const cid of clusterIds) {
        const cnode = condensedNodes[cid];
        const children = Object.values(condensedNodes).filter(n => n.parent === cid);
        let childStabilitySum = 0;
        for (const child of children) {
            childStabilitySum += subtreeStability[child.id] || 0;
        }
        if (childStabilitySum > cnode.stability) {
            subtreeStability[cid] = childStabilitySum;
            cnode.selected = false;
        } else {
            subtreeStability[cid] = cnode.stability;
            cnode.selected = true;
            const queue = [...children];
            while (queue.length > 0) {
                const qnode = queue.shift();
                qnode.selected = false;
                queue.push(...Object.values(condensedNodes).filter(n => n.parent === qnode.id));
            }
        }
    }

    const labels = new Array(n).fill(-1);
    for (const cid of Object.keys(condensedNodes)) {
        const cnode = condensedNodes[cid];
        if (cnode.selected) {
            const lbl = cid === "0" ? 1 : parseInt(cid);
            for (const pt of cnode.points) {
                labels[pt] = lbl;
            }
        }
...
// ── Recursive K-Medoids Cluster Splitting ─────────────────────────────────
function splitClusterKMedoids(trackIndices, distanceMatrix, limit) {
    if (trackIndices.length <= limit) {
        return [trackIndices];
    }

    const n = trackIndices.length;
    // Step 1: Initialize medoids by choosing the two points that are furthest apart
    let m1 = trackIndices[0];
    let m2 = trackIndices[0];
    let maxDist = -1;
    for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
            const d = distanceMatrix[trackIndices[i]][trackIndices[j]];
            if (d > maxDist) {
                maxDist = d;
                m1 = trackIndices[i];
                m2 = trackIndices[j];
    ...
    // If all points are at distance 0, just split arbitrary half
    if (maxDist <= 0) {
        const mid = Math.floor(n / 2);
        const part1 = trackIndices.slice(0, mid);
        const part2 = trackIndices.slice(mid);
        return [
            ...splitClusterKMedoids(part1, distanceMatrix, limit),
            ...splitClusterKMedoids(part2, distanceMatrix, limit)
        ];
    }
    ...
    // Run K-medoids for up to 10 iterations
    for (let iter = 0; iter < 10; iter++) {
        ...
        for (const idx of trackIndices) {
            const d1 = distanceMatrix[idx][m1];
            const d2 = distanceMatrix[idx][m2];
            if (d1 <= d2) {
                cluster1.push(idx);
            } else {
                cluster2.push(idx);
        ...
        // Handle empty/trivial clusters to avoid infinite loops
        ...
        // Update medoids
        let bestM1 = m1;
        let minDistSum1 = Infinity;
        for (const candidate of cluster1) {
            let sumDist = 0;
            for (const other of cluster1) {
                sumDist += distanceMatrix[candidate][other];
            }
            if (sumDist < minDistSum1) {
                minDistSum1 = sumDist;
                bestM1 = candidate;
            }
        }
        ...
        let bestM2 = m2;
        let minDistSum2 = Infinity;
        for (const candidate of cluster2) {
            let sumDist = 0;
            for (const other of cluster2) {
                sumDist += distanceMatrix[candidate][other];
            }
            if (sumDist < minDistSum2) {
                minDistSum2 = sumDist;
                bestM2 = candidate;
            }
        }

        if (bestM1 === m1 && bestM2 === m2) {
            break;
        }
        m1 = bestM1;
        m2 = bestM2;
    ...
    // Safeguard: if K-medoids failed to split (one cluster is empty or has all points)
    if (cluster1.length === 0 || cluster2.length === 0 || cluster1.length === n || cluster2.length === n) {
        const mid = Math.floor(n / 2);
        cluster1 = trackIndices.slice(0, mid);
        cluster2 = trackIndices.slice(mid);
    }

    // Recursively split the sub-clusters if they still exceed the limit
    return [
        ...splitClusterKMedoids(cluster1, distanceMatrix, limit),
        ...splitClusterKMedoids(cluster2, distanceMatrix, limit)
    ];
}

// ── Group and Cluster Tracks Wrapper ──────────────────────────────────────
function groupAndClusterTracks(basinFiltered, numMembers, datasetName) {
    if (basinFiltered.length === 0) return { clusters: [], trackLabels: [] };
    const n = basinFiltered.length;
    ...
    for (let i = 0; i < n; i++) {
        for (let j = i; j < n; j++) {
            if (i === j) {
                ...
                const dist = getTrackDistance(basinFiltered[i], basinFiltered[j]);
    ...
    // Allow smaller but physically meaningful splits (e.g., 3% for large ensembles, 8% for base ensembles, min size of 4 tracks)
    const minClusterSize = Math.max(4, Math.round(numMembers * (numMembers > 100 ? 0.03 : 0.08)));
    const labels = runHDBSCAN(distanceMatrix, minClusterSize);

    const clustersByLabel = {};
    for (let i = 0; i < n; i++) {
        const label = labels[i];
        if (label === -1) continue;
        if (!clustersByLabel[label]) clustersByLabel[label] = [];
    ...
    const limit = getDatasetLimit(datasetName);
    ...
    for (const [label, trackIndices] of Object.entries(clustersByLabel)) {
        if (trackIndices.length > limit) {
            ...
            finalGroups.push(trackIndices);
        }
    }

    const clusters = [];
    let nextDistId = 1;
    for (const trackIndices of finalGroups) {
        const clusterTracks = trackIndices.map(idx => basinFiltered[idx]);
        const origins = clusterTracks.map(t => t[0]);
        const minH = Math.min(...origins.map(o => o.h));
        const maxH = Math.max(...origins.map(o => o.h));
        const center = meanGeoCenter(origins);
        clusters.push({
            distId: nextDistId++,
            origins,
            minH,
    ...
    const trackLabels = new Array(n).fill(null);
    for (const c of clusters) {
        for (const idx of c.trackIndices) {
            trackLabels[idx] = c.distId;
        }
    }
    return { clusters, trackLabels };
}
...
function parseCycleStats(csvText, pairedCsvText, datasetName, basinName, horizonName) {
    ...
    // Group raw rows by ensemble member (sample)
    const memberGroups = {};
    for (const row of rawRows) {
        ...
        const sample = row.sample || "default";

        if (!memberGroups[sample]) memberGroups[sample] = [];
        memberGroups[sample].push({ lat, lon: llon, p: pres, windKmh, h: leadH, initTime, sample });
    }

    // Stitch candidate centers into tracks for each ensemble member
    const stitchedTracks = [];
    for (const sample of Object.keys(memberGroups)) {
        const memberRows = memberGroups[sample];
        const memberTracks = stitchMemberTracks(memberRows);
        stitchedTracks.push(...memberTracks);
    }

    // Keep only tracks whose origin point falls inside the selected basin
    const basinFiltered = stitchedTracks.filter(points => {
    ...
    const numMembers = new Set(rawRows.map(r => r.sample || "default")).size;
    const { clusters, trackLabels } = groupAndClusterTracks(basinFiltered, numMembers, datasetName);

    const tracksByDist = {};
    basinFiltered.forEach((track, idx) => {
        const distId = trackLabels[idx];
        if (distId !== null) {
            if (!tracksByDist[distId]) tracksByDist[distId] = [];
            tracksByDist[distId].push(track);
        }
    });
    ...
    let initDate = null;
    const firstPointWithInit = Object.values(memberGroups)[0]?.[0];
    ...
    disturbanceList.sort((a, b) => {
        if (a.minH !== b.minH) return a.minH - b.minH;
    ...
    for (const dist of disturbanceList) {
        const tracks = updatedTracksByDist[dist.id] || [];
        let minRequiredMembers = 25;
        if (datasetName === "large") minRequiredMembers = 100;
        else if (datasetName === "aigefs") minRequiredMembers = 8;
        ...
        for (const h of hours) {
            ...
            if (usePaired) {
                ...
                const ptsAtHour = d.lats.map((lat, idx) => ({ lat, lon: d.lons[idx] }));
                const geoMean = meanGeoCenter(ptsAtHour);
                mLat = geoMean.lat;
                mLon = geoMean.lon;
...
export default function SpaghettiPlot() {
    ...
    const [dataset, setDataset] = useState("fnv3p2");
    ...
    const matchDisturbancesAcrossCycles = useCallback((selectedDist, cyclesData) => {
        ...
        for (let i = 0; i < cyclesData.length; i++) {
            ...
            if (i === 0) {
                // For the latest cycle, the match is exactly the selected disturbance
                match = cycle.disturbances.find(d => d.id === selectedDist.id) || selectedDist;
            } else {
        ...
        const loadAllCycles = async () => {
            ...
            try {
                ...
                for (const c of activeCycles) {
                    ...
                    const csvText = decodeObfuscatedData(encTracks, dataset === "aigefs" ? "CalauanWeather2026" : 0xAA);
                    ...
                    if (c.paired) {
                        try {
                            ...
                            if (pairedRes.ok) {
                                ...
                                pairedCsvText = decodeObfuscatedData(encPaired, dataset === "aigefs" ? "CalauanWeather2026" : 0xAA);
                            }
                        } catch (_) { }
                    }
                ...
                if (!cancelled) {
                    ...
                    if (!reselected) {
                        let targetDist = null;
                        if (activeDisturbanceId !== null && parsedCycles.length > 0) {
                            targetDist = parsedCycles[0].disturbances.find(d => d.id === activeDisturbanceId);
                        }
                        if (!targetDist && parsedCycles.length > 0 && parsedCycles[0].disturbances.length > 0) {
                            targetDist = parsedCycles[0].disturbances[0];
                        }
                        if (targetDist) {
                            const chain = matchDisturbancesAcrossCycles(targetDist, parsedCycles);
                            setSelectedTrendSystem(chain);
                            setActiveDisturbanceId(targetDist.id);
                        } else {
                            setSelectedTrendSystem(null);
    ...
    const loadData = useCallback(async () => {
        ...
        let pairedUrl = LOCAL_CSV[`${dataset}Paired`] || null;

        if (dataset === "fnv3p2" || dataset === "large") {
            try {
        ...
        try {
            ...
            csvText = decodeObfuscatedData(encryptedText, dataset === "aigefs" ? "CalauanWeather2026" : 0xAA);
        } catch (err) {
            setStatus("error");
            setStatusMsg(
        ...
        // Base → fnv3p2_paired_latest.csv, Large → fnv3_large_paired_latest.csv
        try {
            const pairedRes = await fetch(getAssetUrl(`${pairedUrl}?t=${Date.now()}`));
            if (pairedRes.ok) {
                const encPairedText = await pairedRes.text();
                pairedCsvText = decodeObfuscatedData(encPairedText, dataset === "aigefs" ? "CalauanWeather2026" : 0xAA);
            }
        } catch (_) {
        ...
        try {
            ...
            const numMembers = new Set(rawRows.map(r => r.sample || "default")).size;
            const { clusters, trackLabels } = groupAndClusterTracks(basinFiltered, numMembers, dataset);
            ...
            for (let trackIndex = 0; trackIndex < basinFiltered.length; trackIndex++) {
                ...
                const distId = trackLabels[trackIndex]; // from HDBSCAN
            ...
            // ── Renumber disturbances deterministically (ascending by minH, then descending by trackCount, then lat, then lon) ──────────
            disturbanceList.sort((a, b) => {
                if (a.minH !== b.minH) return a.minH - b.minH;
                if (b.trackCount !== a.trackCount) return b.trackCount - a.trackCount;
                if (b.lat !== a.lat) return b.lat - a.lat;
            ...
            if (animObjectsRef.current) {
                animObjectsRef.current.forEach(obj => {
                    if (obj.distId != null && oldToNew[obj.distId] != null) {
                        obj.distId = oldToNew[obj.distId];
                    }
                });
            }
            ...
            if (meanLayerGroupRef.current) {
                for (const dist of disturbanceList) {
                    const tracks = tracksByDisturbance[dist.id] || [];
                    let minRequiredMembers = 25;
                    if (dataset === "large") minRequiredMembers = 100;
                    else if (dataset === "aigefs") minRequiredMembers = 8;
                    ...
                    for (const h of hours) {
                        ...
                        if (usePaired) {
                            ...
                            if (semLats.length > 0) {
                                const ptsAtHour = semLats.map((lat, idx) => ({ lat, lon: semLons[idx] }));
                                const geoMean = meanGeoCenter(ptsAtHour);
                                mLat = geoMean.lat;
                                mLon = geoMean.lon;
                            } else {
    ...
    const exportScreenshot = async () => {
        ...
        try {
            ...
            a.href = url;
            const modelPrefix = dataset === 'ifs' ? 'ECMWF' : dataset === 'aifs' ? 'ECMWF-AIFS' : dataset === 'aigefs' ? 'NOAA-AIGEFS' : dataset === 'large' ? 'GDM-FNV3-Large' : dataset === 'fnv3p2' ? 'GDM-FNV3-Base' : dataset === 'fnv3p1' ? 'GDM-FNV3P1' : dataset === 'oper' ? 'GDM-OPER' : 'GDM-FNV3';
    ...
    const sidebarContent = (
        <div className="spaghetti-sidebar">
            ...
            <div className="spaghetti-header">
                <div className="spaghetti-header-top">
                    <h1 className="spaghetti-title">
                        {viewMode === "tracker" ? "Ensemble Tracker" :
                            viewMode === "animation" ? "Animation mode" :
                                viewMode === "filter" ? "Ensemble filter" :
                                    viewMode === "trends" ? "Run Cycle trend" :
                                        "Ensemble Tracker"}
                    </h1>
            ...
            <div>
                ...
                <div className="segmented-control">
                    ...
                    { id: "trends", label: "Trends" }]
                        .map(opt => {
                            const isTrendsLocked = opt.id === "trends" && dataset !== "fnv3p2" && dataset !== "large";
                            return (
                                <button
                                    key={opt.id}
                                    onClick={() => {
            ...
            <div>
                ...
                <div className="segmented-control">
                    {[{ id: "fnv3p2", label: "FNV3 Base" },
                    { id: "fnv3p1", label: "FNV3P1" },
                    { id: "oper", label: "OPER" },
                    ...
                    { id: "aigefs", label: "AI-GEFS" }]
                        .map(opt => {
                            ...
                            const isTrendsLocked = viewMode === "trends" && opt.id !== "fnv3p2" && opt.id !== "large";
            ...
            <div className="spaghetti-footer">
                <p className="spaghetti-footer-text">
                    ...
                    Data: {dataset === "ifs" ? "ECMWF IFS Ensemble" : dataset === "aifs" ? "ECMWF AIFS Ensemble" : dataset === "aigefs" ? "AI-GEFS Ensemble" : dataset === "large" ? "FNV3 Large Ensemble" : dataset === "fnv3p2" ? "GDM FNV3 Base (FNV3P2)" : dataset === "fnv3p1" ? "GDM FNV3P1" : dataset === "oper" ? "GDM OPER" : "GDM FNV3"}<br />
                    ...
                    {(viewMode === "animation" || (isExporting && viewMode !== "trends")) && (
                        <div className="gif-watermark">
                            <h3 className="gif-watermark-title">
                                {dataset === 'ifs' ? 'ECMWF IFS Ensemble Track' : dataset === 'aifs' ? 'ECMWF AIFS Ensemble Track' : dataset === 'aigefs' ? 'AI-GEFS Ensemble Track' : dataset === 'large' ? 'Google Deepmind FNV3 1000 Ensemble Track' : dataset === 'fnv3p2' ? 'Google Deepmind FNV3 Base Ensemble Track' : dataset === 'fnv3p1' ? 'Google Deepmind FNV3P1 Ensemble Track' : dataset === 'oper' ? 'Google Deepmind OPER Ensemble Track' : 'Google Deepmind FNV3 Ensemble Track'}
                            </h3>
