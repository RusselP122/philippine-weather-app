            ...
            if (item.type === "support") {
              return (
                <li key={index}>
                  <a
                    href={item.href}
                    className="px-3.5 py-1.5 rounded-full bg-gradient-to-r from-rose-500 via-rose-600 to-amber-500 text-white font-extrabold hover:from-rose-600 hover:to-amber-600 transition-all text-xs uppercase tracking-wide shadow-md flex items-center gap-1.5 border border-rose-400/40"
                  >
        ...
        <div className="lg:hidden border-t border-sky-100 bg-white/95 backdrop-blur-md h-screen overflow-y-auto pb-20">
          <div className="px-4 py-4 space-y-1">
            {navItems.map((item, index) => {
              ...
              if (item.type === "support") {
                return (
                  <a key={index} href={item.href} className="block mt-3 text-center py-2.5 px-4 bg-gradient-to-r from-rose-500 to-amber-500 text-white rounded-lg font-bold shadow-md uppercase tracking-wider text-xs flex items-center justify-center gap-1.5">
                    <span>Support Us</span>
                    <span>❤️</span>
                  </a>
                );
