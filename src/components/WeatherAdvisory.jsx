...
const WeatherAdvisory = () => {
  ...
  useEffect(() => {
    // Fetch GeoJSON map of provinces
    ...
    // Fetch advisory data generated by generate_advisory_data.py with cache-busting
    fetch(`/data/advisory_data.json?t=${Date.now()}`)
      .then((res) => {
        if (!res.ok) throw new Error("Network response was not ok");
        return res.json();
      })
    ...
    // Auto-refresh JAXA GSMaP data every 10 minutes
  ...
  const selectProvinceByName = (provName) => {
    if (!mapData) return;
    const feature = mapData.features.find((f) => f.name.toLowerCase() === provName.toLowerCase());
    if (!feature) return;

    const provData = currentDayData?.provinces?.[feature.name] || {};
    const val = activeHazard === "Wind" ? (provData.wind_kph || 0) : (provData.rainfall !== undefined ? provData.rainfall : (provData.rainfall_mm || 0));

    setSelectedProvince({
      ...feature,
      ...provData,
      rainfall: provData.rainfall !== undefined ? provData.rainfall : (provData.rainfall_mm || 0),
      alert: getAlertLevel(val)
    });

    if (feature.group && activeRegion !== feature.group) {
      setActiveRegion(feature.group);
    }
    setSearchQuery("");
  ...
  const getAlertLevel = (val) => {
    ...
    if (activeHazard === "Wind") {
      ...
      return { title: "NO WARNING", style: "bg-slate-900/60 text-slate-400 border-slate-800/40", text: "Standard background wind conditions." };
    }
    ...
    if (cat === "red") return { title: "Severe Red Warning", style: "bg-red-500/10 text-red-400 border-red-500/25", text: "Severe flooding and widespread landslides expected. Evacuate immediately." };
    if (cat === "orange") return { title: "Heavy Orange Alert", style: "bg-orange-500/10 text-orange-400 border-orange-500/25", text: "Flooding is highly likely. Residents near rivers and slopes should prepare." };
    if (cat === "yellow") return { title: "Moderate Yellow Advisory", style: "bg-yellow-500/10 text-yellow-400 border-yellow-500/25", text: "Localized minor flooding is possible in low-lying areas." };
    ...
    return { title: "No Warnings Active", style: "bg-slate-900/60 text-slate-400 border-slate-800/40", text: "Standard background levels. Scattered light rains or fair weather." };
  };

  const resolvedSelectedProvince = useMemo(() => {
    if (!selectedProvince) return null;
    const provData = currentDayData?.provinces?.[selectedProvince.name] || {};
    ...
    return {
      ...
      ...provData,
      rainfall,
      alert: getAlertLevel(hazardVal)
    };
  }, [selectedProvince, currentDayData, activeHazard, selectedMode]);

  const activeDisplayProvince = hoveredProvince || resolvedSelectedProvince;
  const isAllClear = warningMetrics.red === 0 && warningMetrics.orange === 0 && warningMetrics.yellow === 0 && warningMetrics.lightBlue === 0;
  ...
  const exportMapAsPNG = () => {
    ...
    try {
      ...
      Object.entries(currentDayData.provinces).forEach(([name, p]) => {
        const val = activeHazard === "Wind" ? (p.wind_kph || 0) : (p.rainfall !== undefined ? p.rainfall : (p.rainfall_mm || 0));
        const cat = activeHazard === "Wind" ? getWindCategory(val) : getCategory(val);
        if (cat === "red") redProvinces.push(name);
        else if (cat === "orange") orangeProvinces.push(name);
        else if (cat === "yellow") yellowProvinces.push(name);
        else if (cat === "lightBlue") lightBlueProvinces.push(name);
      });
      ...
      const isWind = activeHazard === "Wind";
      const headerTitle = selectedMode === "realtime"
        ? "REALTIME SATELLITE RAIN"
        : (isWind ? `24-HR MAX WIND GUSTS (DAY ${selectedDay})` : `24-HR RAINFALL (DAY ${selectedDay})`);

      titleGroup.innerHTML = `
        <rect x="0" y="0" width="370" height="240" rx="14" fill="rgba(9, 13, 22, 0.9)" stroke="rgba(255, 255, 255, 0.15)" stroke-width="1.2" />
        <text x="25" y="40" fill="${isWind ? '#c084fc' : (selectedMode === 'realtime' ? '#34d399' : '#38bdf8')}" font-size="14" font-weight="bold" letter-spacing="1.2">${headerTitle}</text>
        <text x="25" y="65" fill="#f1f5f9" font-size="12" font-weight="bold" font-family="sans-serif">${advisoryData?.system || "AI Multi-Model Consensus"}</text>
        <line x1="25" y1="80" x2="345" y2="80" stroke="rgba(255, 255, 255, 0.12)" stroke-width="1" />
        <text x="25" y="108" fill="#e2e8f0" font-size="11" font-weight="bold" font-family="sans-serif">${selectedMode === 'realtime' ? 'OBSERVATION TIMEFRAME:' : 'FORECAST VALIDITY WINDOW:'}</text>
        <text x="25" y="132" fill="${isWind ? '#c084fc' : '#38bdf8'}" font-size="11" font-weight="bold">${currentDayData?.validity?.split(" to ")?.[0] || ""}</text>
        <text x="25" y="152" fill="#64748b" font-size="10" font-weight="bold">${selectedMode === 'realtime' ? '' : 'to'}</text>
        <text x="25" y="172" fill="${isWind ? '#c084fc' : '#38bdf8'}" font-size="11" font-weight="bold">${currentDayData?.validity?.split(" to ")?.[1] || ""}</text>
        <line x1="25" y1="192" x2="345" y2="192" stroke="rgba(255, 255, 255, 0.08)" stroke-width="1" />
        <text x="25" y="215" fill="#64748b" font-size="10.5">Model Run: ${advisoryData?.init_time || "Latest"}</text>
      `;
      ...
      legendGroup.innerHTML = `
        ...
        <text x="25" y="38" fill="${isWind ? '#c084fc' : '#38bdf8'}" font-size="13" font-weight="bold" letter-spacing="1.5" font-family="monospace">${isWind ? 'WIND THREAT LEGEND' : 'WARNING LEVEL LEGEND'}</text>
        ...
        <text x="62" y="85" fill="#f1f5f9" font-size="13" font-weight="bold">${isWind ? 'Typhoon Alert (>118 kph)' : (selectedMode === 'realtime' ? 'Torrential Rain (>30 mm/hr)' : 'Red Warning (200+ mm)')}</text>
        <text x="62" y="102" fill="#94a3b8" font-size="10.5">${isWind ? 'Extreme damage to structures expected' : (selectedMode === 'realtime' ? 'Dangerous torrential downpour' : 'Severe widespread flooding & landslides')}</text>
        <rect x="25" y="124" width="24" height="24" rx="6" fill="#F97316" stroke="rgba(255, 255, 255, 0.2)" stroke-width="0.5" />
        <text x="62" y="141" fill="#f1f5f9" font-size="13" font-weight="bold">${isWind ? 'Severe TS Alert (89 - 117 kph)' : (selectedMode === 'realtime' ? 'Heavy Rain (15-30 mm/hr)' : 'Orange Alert (100 - 200 mm)')}</text>
        <text x="62" y="158" fill="#94a3b8" font-size="10.5">${isWind ? 'Significant wind damage possible' : (selectedMode === 'realtime' ? 'High intensity rain' : 'High risk of flooding & soil slides')}</text>
        <rect x="25" y="180" width="24" height="24" rx="6" fill="#EAB308" stroke="rgba(255, 255, 255, 0.2)" stroke-width="0.5" />
        <text x="62" y="197" fill="#f1f5f9" font-size="13" font-weight="bold">${isWind ? 'Tropical Storm (62 - 88 kph)' : (selectedMode === 'realtime' ? 'Moderate Rain (7.5-15 mm/hr)' : 'Yellow Advisory (50 - 100 mm)')}</text>
        <text x="62" y="214" fill="#94a3b8" font-size="10.5">${isWind ? 'Moderate wind threat, secure items' : (selectedMode === 'realtime' ? 'Moderate steady rain' : 'Localized pooling & minor flooding')}</text>
        <rect x="25" y="236" width="24" height="24" rx="6" fill="#38BDF8" stroke="rgba(255, 255, 255, 0.2)" stroke-width="0.5" />
        <text x="62" y="253" fill="#f1f5f9" font-size="13" font-weight="bold">${isWind ? 'Depression Alert (39 - 61 kph)' : (selectedMode === 'realtime' ? 'Light Rain (2.5-7.5 mm/hr)' : 'Light Blue Alert (25 - 50 mm)')}</text>
        <text x="62" y="270" fill="#94a3b8" font-size="10.5">${isWind ? 'Breezy conditions, minor impacts' : (selectedMode === 'realtime' ? 'Light steady rain' : 'Localized minor ponding & wet roads')}</text>
        <rect x="25" y="292" width="24" height="24" rx="6" fill="#334155" stroke="rgba(255, 255, 255, 0.2)" stroke-width="0.5" />
        <text x="62" y="309" fill="#cbd5e1" font-size="13" font-weight="bold">${isWind ? 'Normal / Calm (< 39 kph)' : (selectedMode === 'realtime' ? 'Clear / Trace (< 2.5 mm/hr)' : 'Light / No Warning (< 25 mm)')}</text>
        <text x="62" y="326" fill="#64748b" font-size="10.5">Standard background conditions</text>
      `;
      clonedSvg.appendChild(legendGroup);

      // 3. Warning Metrics Summary Box
      ...
      metricsGroup.innerHTML = `
        ...
        <text x="25" y="38" fill="${isWind ? '#c084fc' : '#38bdf8'}" font-size="13" font-weight="bold" letter-spacing="1.5" font-family="monospace">PROVINCIAL WARNING METRICS</text>
        <line x1="25" y1="50" x2="345" y2="50" stroke="rgba(255, 255, 255, 0.12)" stroke-width="1" />
      ...
      img.onload = () => {
        ...
        downloadLink.download = `PH_Weather_Advisory_${activeHazard}_${new Date().toISOString().split('T')[0]}.png`;
  ...
  if (!geoData || !advisoryData) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-64px)] w-full bg-slate-950 text-slate-300">
        ...
        <p className="mt-2 text-xs text-slate-500 font-mono">Loading dynamic province map overlays & AI Multi-Model consensus...</p>
      </div>
    );
  }
    ...
    <div className="relative h-[calc(100vh-64px)] w-full bg-slate-950 font-sans overflow-hidden flex flex-col md:flex-row">
      ...
      <div className="flex md:hidden bg-slate-900 border-b border-slate-800/80 text-xs font-bold text-slate-400 z-30 shrink-0">
        <button
          ...
          className={`flex-1 py-3.5 flex items-center justify-center gap-2 border-b-2 transition-all ${activeTab === "map"
        ...
        <button
          ...
          className={`flex-1 py-3.5 flex items-center justify-center gap-2 border-b-2 transition-all ${activeTab === "bulletin"
              ? "border-sky-500 text-white bg-slate-950/45"
              : "border-transparent hover:text-slate-200"
            }`}
        >
      ...
      <div
        ...
        className={`relative w-full bg-[#020617] overflow-hidden flex items-center justify-center transition-all duration-300 ${activeTab === "map" ? "flex flex-1 h-[calc(100vh-112px)]" : "hidden"
      ...
      >
        ...
        {mapData && (
          ...
          >
            ...
            {/* Mask overlays for Laguna de Bay and Taal Volcano */}
        ...
        {/* ── TOP CONTROLS & SEARCH BAR OVERLAY ── */}
        <div className="absolute top-4 left-4 right-4 z-20 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2 pointer-events-none">
          <div className="flex flex-wrap items-center gap-2 pointer-events-auto">
            ...
            <div className="bg-slate-950/85 backdrop-blur-md p-1 rounded-lg border border-slate-800/80 shadow-xl flex gap-1">
              {["All", "Luzon", "Visayas", "Mindanao"].map((region) => (
                <button
                  key={region}
                  onClick={() => setActiveRegion(region)}
                  className={`px-2.5 py-1.5 rounded text-[10px] sm:text-xs font-bold tracking-wide uppercase cursor-pointer transition-all duration-300 ${activeRegion === region
            ...
            <button
              ...
              className={`px-2.5 py-1.5 rounded-lg text-[10px] sm:text-xs font-semibold backdrop-blur-md border shadow-lg flex items-center justify-center gap-1.5 cursor-pointer transition-all duration-300 ${showLabels
                  ? "bg-emerald-600/90 text-white border-emerald-500"
                  : "bg-slate-950/85 text-slate-400 border-slate-800 hover:bg-slate-900/80"
                }`}
            >
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg text-[10px] sm:text-xs font-semibold backdrop-blur-md border shadow-lg flex items-center justify-center gap-1.5 cursor-pointer bg-slate-950/85 text-slate-400 border-slate-800 hover:bg-slate-900/80 hover:text-sky-400 transition-all duration-300"
            >
          ...
          {/* Quick Province Search Bar */}
          <div className="relative pointer-events-auto w-full md:w-64">
            <div className="flex items-center bg-slate-950/90 border border-slate-800 backdrop-blur-md rounded-lg px-2.5 py-1.5 shadow-xl focus-within:border-sky-500 transition-colors">
              ...
              <input
                ...
                className="bg-transparent text-xs text-slate-100 placeholder-slate-500 focus:outline-none w-full font-medium"
              />
              {searchQuery && (
                <button
                  onClick={() => {
            ...
            {isSearchOpen && filteredProvincesList.length > 0 && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-slate-950/95 border border-slate-800 rounded-lg shadow-2xl backdrop-blur-xl overflow-hidden z-50 max-h-56 overflow-y-auto">
                {filteredProvincesList.map((f) => {
                  const pData = currentDayData?.provinces?.[f.name] || {};
                  const pRain = pData.rainfall !== undefined ? pData.rainfall : (pData.rainfall_mm || 0);
                  const pVal = activeHazard === "Wind" ? (pData.wind_kph || 0) : pRain;
                  const pCat = activeHazard === "Wind" ? getWindCategory(pVal) : getCategory(pVal);

                  return (
                    <button
                      key={f.id}
                      onClick={() => selectProvinceByName(f.name)}
                      className="w-full px-3 py-2 text-left hover:bg-slate-900/80 border-b border-slate-900 flex items-center justify-between transition-colors cursor-pointer"
                    >
                      <div>
                        <span className="text-xs font-bold text-slate-100 block">{f.name}</span>
                        <span className="text-[10px] text-slate-400">{f.group} • {f.region.split("(")[0].trim()}</span>
                      </div>
                      <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border ${pCat === 'red' ? 'bg-red-500/20 text-red-400 border-red-500/40' :
                          pCat === 'orange' ? 'bg-orange-500/20 text-orange-400 border-orange-500/40' :
                            pCat === 'yellow' ? 'bg-yellow-500/20 text-yellow-400 border-yellow-500/40' :
                              pCat === 'lightBlue' ? 'bg-sky-500/20 text-sky-400 border-sky-500/40' :
                                'bg-slate-800/40 text-slate-400 border-slate-700/40'
                        }`}>
                        {Math.round(pVal)} {activeHazard === "Wind" ? "kph" : (selectedMode === "realtime" ? "mm/h" : "mm")}
                      </span>
                    </button>
                  );
                })}
        ...
        {selectedMode === "realtime" && (
          <div className="hidden md:flex absolute top-16 right-4 z-20 pointer-events-none bg-slate-950/90 border border-emerald-500/40 text-emerald-300 backdrop-blur-md px-3.5 py-2 rounded-xl shadow-2xl items-center gap-2.5 font-mono text-[10px] font-bold">
        ...
        {isTooltipVisible && hoveredProvince && (
          <div
            className="hidden md:flex absolute z-50 pointer-events-none w-76 rounded-xl border border-slate-700/60 bg-slate-950/95 p-4 text-slate-200 shadow-2xl backdrop-blur-lg transition-opacity duration-200 flex flex-col gap-2.5"
            style={{
              left: tooltipPos.x,
              top: tooltipPos.y,
          ...
          >
            ...
            <div className="flex justify-between items-start border-b border-slate-800 pb-2">
              <div>
                ...
                <p className="text-[10px] font-semibold text-sky-400 uppercase tracking-wider mt-0.5">
                  {hoveredProvince.group} • {hoveredProvince.region ? hoveredProvince.region.split("(")[0].trim() : ""}
                </p>
              </div>
            ...
            <div className="flex items-center gap-4 bg-slate-900/60 px-3 py-2 rounded-lg border border-slate-800">
              <div className="flex items-center justify-center w-8 h-8 rounded-full bg-sky-500/10 text-sky-400">
                {activeHazard === "Wind" ? <Wind className="w-4 h-4 animate-pulse" /> : <CloudRain className="w-4 h-4 animate-pulse" />}
              </div>
              <div>
                ...
                {activeHazard === "Wind" ? (
                  ...
                  selectedMode === "forecast" && hoveredProvince.min_rainfall !== undefined && hoveredProvince.max_rainfall !== undefined ? (
                    <p className="text-base font-bold text-white tracking-wide leading-tight mt-1">
                      ...
                      <br /><span className="text-[9px] font-normal text-slate-400 block mt-0.5">Consensus Peak: {Math.round(hoveredProvince.rainfall)} mm</span>
                    </p>
                  ) : (
                    <p className="text-base font-bold text-white tracking-wide mt-1">
            ...
            {/* Multi-AI Model Breakdown & Confidence */}
            {hoveredProvince.confidence && (
              <div className="bg-slate-900/40 px-3 py-2 rounded-lg border border-slate-800/80 text-[10px] font-mono flex flex-col gap-1.5">
                <div className="flex justify-between items-center text-slate-400">
                  <span>Ensemble Confidence:</span>
                  <span className={`font-bold ${hoveredProvince.confidence.includes('High') ? 'text-emerald-400' :
        ...
        {resolvedSelectedProvince && (
          <div className="hidden md:flex absolute bottom-4 left-4 z-30 w-84 rounded-xl border border-slate-700/60 bg-slate-950/95 p-4 text-slate-200 shadow-2xl backdrop-blur-lg flex flex-col gap-2.5 animate-in fade-in slide-in-from-bottom-5 duration-300">
            <div className="flex justify-between items-start border-b border-slate-800 pb-2">
              <div>
                <h3 className="font-bold text-slate-100 text-sm tracking-wide">{resolvedSelectedProvince.name}</h3>
                <p className="text-[10px] font-semibold text-sky-400 uppercase tracking-wider mt-0.5">
                  {resolvedSelectedProvince.group} • {resolvedSelectedProvince.region ? resolvedSelectedProvince.region.split("(")[0].trim() : ""}
                </p>
              </div>
            ...
            <div className="flex items-center gap-4 bg-slate-900/60 px-3 py-2 rounded-lg border border-slate-800">
              ...
              <div>
                ...
                {activeHazard === "Wind" ? (
                  <p className="text-base font-bold text-white tracking-wide mt-1">
                    {Math.round(resolvedSelectedProvince.wind_kph || 0)} <span className="text-xs font-normal text-slate-400 font-sans">kph</span>
                  </p>
                ) : (
                  selectedMode === "forecast" && resolvedSelectedProvince.min_rainfall !== undefined && resolvedSelectedProvince.max_rainfall !== undefined ? (
                    <p className="text-base font-bold text-white tracking-wide leading-tight mt-1">
                      {Math.round(resolvedSelectedProvince.min_rainfall)} - {Math.round(resolvedSelectedProvince.max_rainfall)} <span className="text-[10px] font-medium text-slate-400 font-sans">mm/24h</span>
                      <br /><span className="text-[9px] font-normal text-slate-400 block mt-0.5">Consensus Peak: {Math.round(resolvedSelectedProvince.rainfall)} mm</span>
                    </p>
                  ) : (
                    <p className="text-base font-bold text-white tracking-wide mt-1">
                      {Math.round(resolvedSelectedProvince.rainfall)} <span className="text-xs font-normal text-slate-400 font-sans">mm / 24h</span>
                    </p>
                  )
                )}
              </div>
            ...
            {resolvedSelectedProvince.confidence && (
              <div className="bg-slate-900/40 p-2.5 rounded-lg border border-slate-800/80 text-[10px] font-mono flex flex-col gap-1.5">
                <div className="flex justify-between items-center text-slate-400">
                  <span>Confidence:</span>
                  <span className={`font-bold ${resolvedSelectedProvince.confidence.includes('High') ? 'text-emerald-400' :
                      resolvedSelectedProvince.confidence.includes('Medium') ? 'text-yellow-400' : 'text-red-400'
                    }`}>{resolvedSelectedProvince.confidence} ({resolvedSelectedProvince.agreement}% agreement)</span>
                </div>
                {resolvedSelectedProvince.models && (
                  <div className="grid grid-cols-3 gap-1 text-slate-400 text-[9px] border-t border-slate-900 pt-1.5 mt-0.5">
            ...
            <div className={`p-2.5 rounded-lg border text-xs leading-relaxed flex flex-col gap-1 ${resolvedSelectedProvince.alert.style}`}>
              <div className="flex items-center gap-1.5 font-bold uppercase tracking-wider text-[10px]">
                <ShieldAlert className="w-3.5 h-3.5" />
                <span>{resolvedSelectedProvince.alert.title}</span>
              </div>
              <p className="opacity-90">{resolvedSelectedProvince.alert.text}</p>
            </div>
          </div>
        )}
      </div>

      {/* ── SIDEBAR PANEL (WEATHER ALERT DESK STYLE) ── */}
      <div
        className={`w-full md:w-84 lg:w-96 bg-[#050811]/95 backdrop-blur-xl border-t md:border-t-0 md:border-l border-slate-800/80 flex flex-col z-20 overflow-hidden shadow-[-15px_0_35px_rgba(0,0,0,0.6)] ${activeTab === "bulletin" ? "flex flex-1 h-[calc(100vh-112px)]" : "hidden"
          } md:flex md:h-full md:flex-none`}
      >
        {/* Header */}
        <div className="bg-gradient-to-r from-sky-950/80 to-blue-950/80 border-b border-sky-800/40 text-center relative overflow-hidden flex-shrink-0 p-4 sm:p-5">
          ...
          <h1 className="text-base sm:text-lg font-black tracking-widest text-white uppercase flex items-center justify-center gap-2 relative">
            <ShieldAlert className="w-5 h-5 text-sky-400 animate-pulse" />
            <span>{selectedMode === "realtime" ? "JAXA Realtime Rain" : "Weather Advisory"}</span>
          </h1>
          <p className="text-xs font-semibold text-sky-300 uppercase tracking-widest mt-1">
            {selectedMode === "realtime" ? "Satellite Precipitation (GSMaP)" : "5-Day AI Ensemble Bulletin"}
          </p>
          {selectedMode === "forecast" && currentDayData && (
            <div className="mt-3 flex flex-col gap-1 items-center">
              <div className="text-[10px] text-slate-300 font-mono bg-slate-950/70 py-1.5 px-3 rounded-xl border border-slate-800/60 text-center leading-snug max-w-full">
                {currentDayData.validity?.includes(" to ") ? (
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sky-300 font-semibold">{currentDayData.validity.split(" to ")[0]}</span>
                    <span className="text-slate-500 text-[9px]">to</span>
                    <span className="text-sky-300 font-semibold">{currentDayData.validity.split(" to ")[1]}</span>
                  </div>
                ) : (
                  <span>{currentDayData.validity}</span>
                )}
              </div>
        ...
        <div className="px-4 pt-3 pb-2 bg-slate-950/80 border-b border-slate-900 flex flex-col gap-2 flex-shrink-0">
          <div className="grid grid-cols-2 gap-1 p-0.5 rounded-lg bg-slate-900/80 border border-slate-800/80">
            <button
              ...
              className={`py-1.5 px-2 rounded-md text-[10px] font-bold tracking-wide uppercase transition-all flex items-center justify-center gap-1.5 cursor-pointer ${selectedMode === "realtime"
            ...
            <button
              ...
              className={`py-1.5 px-2 rounded-md text-[10px] font-bold tracking-wide uppercase transition-all flex items-center justify-center gap-1.5 cursor-pointer ${selectedMode === "forecast"
          ...
          {selectedMode === "forecast" && (
            <div className="grid grid-cols-2 gap-1 p-0.5 rounded-lg bg-slate-900/80 border border-slate-800/80 mt-1">
              <button
                ...
                className={`py-1.5 px-2 rounded-md text-[10px] font-bold tracking-wide uppercase transition-all flex items-center justify-center gap-1.5 cursor-pointer ${activeHazard === "Rainfall"
                    ? "bg-sky-500 text-white shadow-md shadow-sky-500/20"
                    : "text-slate-400 hover:text-slate-200"
                  }`}
              >
              ...
              <button
                ...
                className={`py-1.5 px-2 rounded-md text-[10px] font-bold tracking-wide uppercase transition-all flex items-center justify-center gap-1.5 cursor-pointer ${activeHazard === "Wind"
              ...
              <div className="grid grid-cols-5 gap-1 p-0.5 rounded-lg bg-slate-900/60 border border-slate-800/60">
                {(realtimeData?.frames || [
                  ...
                  <button
                    ...
                    className={`py-1.5 rounded-md text-[9px] font-bold transition-all cursor-pointer ${selectedRealtimeFrameIdx === idx
              ...
              <div className="grid grid-cols-5 gap-1 p-0.5 rounded-lg bg-slate-900/60 border border-slate-800/60">
                {[1, 2, 3, 4, 5].map((d) => (
                  <button
                    ...
                    className={`py-1.5 rounded-md text-[10px] sm:text-xs font-bold transition-all cursor-pointer ${selectedDay === d
        ...
        <div className="p-5 flex-1 overflow-y-auto space-y-4">
          ...
          {/* ALL CLEAR NOTIFICATION IF 0 WARNINGS */}
          {isAllClear && (
            <div className="p-4 rounded-xl bg-emerald-950/30 border border-emerald-500/30 flex items-start gap-3">
              <ShieldCheck className="w-6 h-6 text-emerald-400 flex-shrink-0 mt-0.5" />
              <div>
                <h3 className="text-xs font-bold text-emerald-300 uppercase tracking-wide">All Clear — Fair Conditions</h3>
                <p className="text-[11px] text-slate-300 mt-1 leading-relaxed">
                  No critical weather warnings active across the Philippine Area of Responsibility for this period. Standard background weather prevailing.
                </p>
              </div>
            </div>
          )}

          <h2 className="text-slate-400 font-bold uppercase tracking-wider text-[11px] flex items-center gap-1.5 border-b border-slate-800/80 pb-2 mb-3">
                    ...
                    {warningProvinces.red.map(p => {
                      ...
                      return (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            selectProvinceByName(p);
                            if (window.innerWidth < 768) {
                              setActiveTab("map");
                            }
                          }}
                          className={`px-2 py-0.5 text-[9.5px] font-mono rounded cursor-pointer transition-all duration-200 border ${isSelected
          ...
          <div className="bg-slate-950/40 rounded-xl overflow-hidden border border-yellow-950 shadow-lg group transition-all duration-300 hover:border-yellow-900/60">
            ...
            <div className="p-4 text-xs text-slate-400 leading-relaxed space-y-2">
              ...
              <div className="flex items-start gap-2">
                <ChevronRight className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />
                <p><strong className="text-slate-200">Landslides:</strong> Minor slips on vulnerable slopes.</p>
              </div>
              <div className="flex items-start gap-2">
                <ChevronRight className="w-3 h-3 text-yellow-500 mt-0.5 flex-shrink-0" />
                <p><strong className="text-slate-200">Action:</strong> Stay tuned to advisory updates.</p>
              </div>
          ...
          <div className="bg-slate-950 p-4 border-t border-slate-900 flex flex-col gap-2 relative flex-shrink-0">
            ...
            <div className="flex items-center justify-center gap-3 mt-1 pb-1">
              ...
              <div className="text-[9px] text-slate-500 font-mono leading-tight">
                ...
                AI Weather Analysis & Forecasting Desk
              </div>
            </div>
          </div>
        )}
      ...
      {activeDisplayProvince && (
        <div className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-slate-950/98 border-t border-slate-800 rounded-t-2xl pb-8 pt-4 px-5 shadow-[0_-10px_35px_rgba(0,0,0,0.85)] backdrop-blur-xl animate-in slide-in-from-bottom-full duration-300">
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div className="flex items-center gap-3 bg-slate-900/60 p-3 rounded-xl border border-slate-800/80">
              <div className="flex items-center justify-center w-9 h-9 rounded-full bg-sky-500/10 text-sky-400 flex-shrink-0">
                {activeHazard === "Wind" ? <Wind className="w-5 h-5 animate-pulse" /> : <CloudRain className="w-5 h-5 animate-pulse" />}
              </div>
              <div>
                ...
                {activeHazard === "Wind" ? (
                  ...
                  selectedMode === "forecast" && activeDisplayProvince.min_rainfall !== undefined && activeDisplayProvince.max_rainfall !== undefined ? (
                    <p className="text-lg font-black text-white tracking-wide leading-tight mt-1">
                      ...
                      <br /><span className="text-[9px] font-normal text-slate-400 block mt-0.5">Consensus: {Math.round(activeDisplayProvince.rainfall)} mm</span>
                    </p>
                  ) : (
                    <p className="text-lg font-black text-white tracking-wide leading-none mt-1">
            ...
            <div className="bg-slate-900/45 p-3 rounded-xl border border-slate-800/80 text-[10px] font-mono flex flex-col justify-center">
              <div className="flex justify-between items-center text-slate-400">
                <span>Confidence:</span>
                <span className={`font-bold ${activeDisplayProvince.confidence === 'High' ? 'text-emerald-400' :
          ...
          {activeDisplayProvince.models && (
            <div className="bg-slate-900/30 px-3 py-2 rounded-lg border border-slate-900 text-[10px] font-mono mb-3">
              <div className="text-slate-500 uppercase tracking-widest text-[8.5px] mb-1">Ensemble Breakdown:</div>
