...
def process_and_plot(model, latest_url, date_str, hour_str, is_base_model):
    ...
    if wp_data.empty:
        print(f"Warning: No data found in the CSV file for {model} for lead_time_hours <= 120.")
        return
    ...
    if paired_local_csv and os.path.exists(paired_local_csv):
        try:
            ...
            p_data = p_data[p_data['lead_time_hours'] <= 120].copy()
    ...
    try:
        ...
        output_file_model = f"{output_dir}/tropical_cyclone_5day_forecast_{model}_{init_time_str}.png"
