...
def get_latest_run_url():
    ...
    for d in dates:
        ...
        for h in hours_desc:
            url = f"{base}/FNV3P2_{date_str}T{h}_00_cyclogenesis.csv"
            try:
    ...
    raise RuntimeError("No available FNV3P2 cyclogenesis runs found in the last 3 days.")
...
try:
    ...
    local_csv = os.path.join(csv_dir, f"FNV3P2_{date_str}T{hour_str}_00_cyclogenesis.csv")
...
num_samples = len(data[data['lead_time_hours'] <= 336]['sample'].unique())
if num_samples == 0:
...
two_day_day = (init_ph + timedelta(days=9)).strftime('%a')
seven_day_day = (init_ph + timedelta(days=14)).strftime('%a')
...
if active_wp_ids:
    ...
    for tid in active_wp_ids:
        ...
        tc_pts_raw = wp_data[(wp_data['track_id'] == tid) & (wp_data['lead_time_hours'] >= 168) & (wp_data['lead_time_hours'] <= 336)]
        if tc_pts_raw.empty:
            continue
...
if len(lons) < 2:
    ...
    if not has_active_tc:
        ...
        end_date_str = (init_ph + timedelta(days=14)).strftime("%m/%d/%Y")
    ...
    disclaimer = (
        ...
        "Please do not treat this as an official forecast. Refer to PAGASA and other official\n"
        "meteorological agencies for official forecasts, warnings, and advisories."
    )
    ...
    # Add title
    ax.set_title("Week 2 Tropical Weather Outlook - Western Pacific", fontsize=16, weight='bold', pad=15)
    ...
    try:
        ...
        output_file = os.path.join(output_dir, "tropical_outlook_week2_latest.png")
...
for i, label in enumerate(unique_labels, start=1):
    ...
    samples_2day = cluster_genesis[cluster_genesis['lead_time_hours'] <= 216]['sample'].unique()
    ...
    area_text = (
        f"Area {i}\n"
        f"Days 8-9 Potential: ({two_day_day}) {cat_2day} ({int(prob_2day_rounded)}%)\n"
        f"Week 2 Potential: ({seven_day_day}) {cat_7day} ({int(prob_7day_rounded)}%)"
    )
    ...
    summary_line = (
        f"Area {i}: {conf_word} confidence of {system_wording} formation during Week 2 (Days 8-14) "
        f"(Stage at genesis: {stage}, Week 2 probability: {int(prob_7day_rounded)}%)"
    )
