...
for init_time in init_times:
    ...
    # Phase 2: Compute genesis average positions over first 48 hours for grouping non-WP candidates
    genesis_starts = {}
    for track_id, samples in validated_tracks.items():
        ...
        starts = []
        for s in samples:
            mask_48 = s['lead_times'] <= 48
            if np.any(mask_48):
                starts.append((np.mean(s['lats'][mask_48]), np.mean(s['lons_shifted'][mask_48])))
            else:
    ...
    for sys in systems:
        ...
        # Tighten clustering eligibility: System must represent >= 5% of the ensemble to be plotted as a cluster
        min_complete_to_cluster = 20 if is_wp else max(50, int(0.05 * total_ensemble_members))
        ...
        if should_cluster:
            ...
            w = np.exp(-0.005 * lead_grid)
            
            dist_matrix = compute_dist_matrix_nan(lats_grid, lons_grid, w)
            
            # Adaptive Epsilon based on median pairwise distance
            valid_dists = dist_matrix[(dist_matrix > 0) & (dist_matrix < 5000.0)]
            if len(valid_dists) > 0:
                median_dist = np.median(valid_dists)
                if is_wp:
                    epsilon = np.clip(median_dist * 0.4, 60.0, 120.0)
                else:
                    epsilon = np.clip(median_dist * 0.5, 120.0, 300.0)
            else:
                epsilon = 100.0 if is_wp else 200.0
            
            # Strict HDBSCAN parameters to suppress spurious scenarios
            if is_wp:
                min_cluster_size = max(25, int(0.04 * n_complete))
                min_samples = max(10, int(0.015 * n_complete))
                allow_single = True
            else:
                min_cluster_size = max(45, int(0.045 * n_complete))
                min_samples = max(15, int(0.015 * n_complete))
                allow_single = False
            ...
            # Second pass on noise only if noise is large enough (>= 300 members)
            noise_mask = (labels == -1)
            ...
            if n_noise >= 300:
                ...
                try:
                    hdb2 = HDBSCAN(
                        min_cluster_size=max(40, int(0.04 * n_noise)),
                        min_samples=max(15, int(0.015 * n_noise)),
                        metric='precomputed',
                        cluster_selection_method='eom',
            ...
            # Centroid-track merging with a larger threshold (300 km)
            ...
            # Post-merge cleaning: Remove small clusters (< 5% within-system probability)
            unique_labels = np.unique(labels)
...
for mode in ['standard', 'cluster']:
    ...
    try:
        ...
        output_file = f"{output_dir}/fnv3_tropical_cyclone_15day_forecast_{init_time_str}{suffix}.png"
